...
4. **Graceful Exit**: Choosing `5` exits the program with a friendly goodbye message.
...
2. **Simple + Compound Interest Calculator**: Compare returns on investments.
3. **Student Grade Manager**: Store multiple students' scores and calculate average, highest, and pass/fail status.
4. **Basic Quiz Game**: Simple multiple-choice question set with scoring systems.
5. **Number System Converter**: Convert decimal numbers into binary equivalents.
6. **Password Strength Checker**: Analyze if a user input password meets safety criteria (length, digits, casing).
7. **Bill Split Calculator**: Divide dinner bill among friends, adding customizable tips.
8. **Unit Converter**: Convert distance (kilometers to miles) and weights.
9. **Simple Login System**: Login portal offering up to 3 retries before locking accounts.
10. **Random Password Generator**: Generate combinations of letters and numbers for digital accounts.
